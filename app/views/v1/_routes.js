...
router.post('/child-name', (req, res) => {
  ...
  if (firstName && lastName) {
    res.redirect('/v1/hs-card/child-dob');
...
router.post('/child-dob', (req, res) => {
  ...
  if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
    ...
    res.redirect('/v1/hs-card/child-updated');
...
router.post('/request-new-pin', (req, res) => {
  ...
  if (newPin == 'yes') {
    res.redirect('/v1/hs-card/receive-new-pin');
  } else if (newPin == 'no') {
    res.redirect('/v1/hs-card/your-healthy-start-card');
  } else {
    res.redirect('/v1/hs-card/request-new-pin');
  }
...
router.post('/receive-new-pin', (req, res) => {
  ...
  if (howToReceive == 'mobile') {
    res.redirect('/v1/personal-details/mobile-phone-number');
