...
router.post('/authenticate-type', (req, res) => {
  ...
  if (authType) {
    res.redirect('/v3/before-you-start/authenticate-details')
  } else {
    res.redirect('/v3/before-you-start/authenticate-type')
  }
...
router.post('/freeze-card', (req, res) => {
  ...
  if (freezeCard == 'yes'){
    res.redirect('/v3/hs-card/card-frozen-or-unfrozen');
  } else if (freezeCard == 'no') {
    res.redirect('/v3/hs-card/lost-or-stolen-card');
  } else {
    res.redirect('/v3/hs-card/freeze-card');
  }
...
router.post('/child-name', (req, res) => {
  ...
  if (firstName && lastName) {
    res.redirect('/v3/hs-card/child-dob');
    // res.redirect('/v3/hs-card/pregnancy-and-children');
  } else {
    res.redirect('/v3/hs-card/child-name');
  }
})
...
router.post('/child-dob', (req, res) => {
  ...
  if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
    ...
    res.redirect('/v3/hs-card/child-updated');
...
router.post('/full-name', (req, res) => {
  ...
  if (firstName && lastName) {
    res.redirect('/v3/personal-details/date-of-birth');
  } else {
    res.redirect('/v3/personal-details/full-name');
  }
...
router.post('/date-of-birth', (req, res) => {
  ...
  if (fullDateOfBirth) {
    res.redirect('/v3/personal-details/address');
  } else {
    res.redirect('/v3/personal-details/date-of-birth');
  }
...
router.post('/address', (req, res) => {
  ...
  if (firstName) {
    if (addressLine1) {
      res.redirect('/v3/personal-details/national-insurance-number');
    } else {
      res.redirect('/v3/personal-details/address');
    ...
    if (addressLine1) {
      res.redirect('/v3/hs-card/cya-new-pin');
    } else {
      res.redirect('/v3/personal-details/address');
...
router.post('/nino', (req, res) => {
  ...
  if (nino) {
    res.redirect('/v3/personal-details/confirmation-personal-details');
  } else {
    res.redirect('/v3/personal-details/national-insurance-number');
  }
...
router.post('/request-new-pin', (req, res) => {
  ...
  if (newPin == 'yes') {
    res.redirect('/v3/hs-card/receive-new-pin');
  } else if (newPin == 'no') {
    res.redirect('/v3/hs-card/your-healthy-start-card');
  } else {
    res.redirect('/v3/hs-card/request-new-pin');
  }
...
router.post('/receive-new-pin', (req, res) => {
  ...
  if (howToReceive == 'mobile') {
    res.redirect('/v3/personal-details/mobile-phone-number');
